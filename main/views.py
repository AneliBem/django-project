...
class CatalogViev(ListView):
    ...
    def get_queryset(self):
        ...
        if size_names:
            queryset = queryset.filter(
                Q(size__name__in=size_names) & Q(sizes__clothingitemsize__available=True))   

        if min_price:
            queryset = queryset.filter(prize__gte=min_price)
        
        if max_price:
            queryset = queryset.filter(prize__lte=max_price)
    ...
    def get_context_data(self, **kwargs):
        ...
        context['catigories'] = Category.objects.all()
...
class ClothingItemDetailView(DetailView):
    ...
    context_object_namen = 'clothing_item'
    slug_field = 'slug'
    slug_url_kwarg = 'slug'
